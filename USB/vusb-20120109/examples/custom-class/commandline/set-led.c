...
const unsigned char rawVid[2] = {0xc0, 0x16}, rawPid[2] = {0xdc, 0x05};
char                vendor[] = {'a', 'r', 'e', 'a', '5', '1', '.', 'n', 'i', 'l', 's', '@', 'g', 'o', 'o', 'g', 'l', 'e', 'm', 'a', 'i', 'l', '.', 'c', 'o', 'm', 0}, product[] = {'H', 'a', 'n', 'd', 's', 'c', 'h', 'u', 'h', 0};
char                buffer[4];
int                 cnt, vid, pid, isOn;
    ...
    if(strcasecmp(argv[1], "status") == 0){
        ...
        cnt = usb_control_msg(handle, USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_OUT, CUSTOM_RQ_SET_STATUS, 1 - isOn, 0, buffer, 0, 5000);
        if(cnt < 0){
            fprintf(stderr, "USB error: %s\n", usb_strerror());
        }
#if ENABLE_TEST
